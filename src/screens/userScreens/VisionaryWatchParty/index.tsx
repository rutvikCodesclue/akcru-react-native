...
const VisionaryWatchParty = ({navigation, route}: WatchPartyViewProps) => {
    ...
    const videoRoomPrivileges = user?.visionaryStatus; // give video room priveleges if its a MIT
    ...
    const [participantMicAccess, setParticipantMicAccess] = useState(false)
    ...
    useEffect(() => {
        ...
        channelA
            ...
            .on('broadcast', {event: 'hand-mic'}, payload => micAccessToggle(payload))
            .subscribe(status => {
    ...
    const micAccessToggle = (payload: any) => {
        if (payload.payload.micAccessGiven) {
            setMicAccess(true)
        } else {
            setMicAccess(false)
        }
    ...
    const toggleParticipantMicAccess = () => {
        console.log('Mic has been handed')
        ...
        channelll.send({
            type: 'broadcast',
            event: 'hand-mic',
            payload: {micAccessGiven: !participantMicAccess},
        })

        setParticipantMicAccess((prev) => !prev)
    }
        ...
        <SafeAreaView style={{marginBottom: SIZES.ScreenHeight / 12}}>
            ...
            {!isFullscreen && (
                <TopContainer
                    ...
                    handleHandMic={toggleParticipantMicAccess}
                    participantMicAccess={participantMicAccess}
                />
            )}
            ...
            <MovieScreen
                ...
                videoRoomPrivileges={videoRoomPrivileges || false}
            />
            ...
            {currentRoomHost ? (
                !isFullscreen ? (
                    <UserVideos
                        ...
                        currentRoomHost={currentRoomHost}
                        members={members.filter(member => member.user.visionaryStatus)}
                        videoRoomPrivileges={videoRoomPrivileges || false}
                    />
                ) : (
                    <WatchPartyDocker
...
export default VisionaryWatchParty;
